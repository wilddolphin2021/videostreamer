...
func main() {
	...
	go encoder(args.InputFormat, args.InputURL, args.OutputURL, args.OutputFormat, args.Audio, args.Verbose, clientChan)
...
func encoder(inputFormat, inputURL string, outputURL string, outputFormat string, audio bool, verbose bool,
	clientChan <-chan *Client) {
	clients := []*Client{}
	var input *Input = nil
	var output *C.struct_VSOutput = nil
	var clientsOnly = len(outputURL) == 0
	...
	for {
		...
		if input == nil {
			if verbose {
				log.Printf("encoder: Opening input...")
			}
		...
		if !clientsOnly {
			if output == nil {
				output = openOutput(outputFormat, outputURL, audio, verbose, input)	
				if output == nil {
					log.Printf("Can't create output")
					return
				}

				if verbose {
					log.Printf("encoder: Opened output")
				}
			}
		}
		...
		if(output != nil) {
			...
			if(!sent) {
				log.Printf("Writing packet failed. Closing output")
			...
			// Send to client(s) not output

			// Write the packet to all clients.
			clientCountBefore = len(clients)

			clients = writePacketToClients(input, &pkt, clients, audio, verbose)
			clientCountAfter = len(clients)
...
func writePacketToClients(input *Input, pkt *C.AVPacket,
	clients []*Client, audio bool, verbose bool) []*Client {
	...
	for _, client := range clients {
		...
		if client.Output == nil {
			...
			client.Output = openOutput(outputFormat, outputURL, audio, verbose, input)
			if client.Output == nil {
...
func writePacketToOutputURL(input *Input, pkt *C.AVPacket, output *C.struct_VSOutput, verbose bool) bool {
	...
	writeRes := C.int(0)
	input.mutex.RLock()
	writeRes = C.vs_write_packet(input.vsInput, output, pktCopy,
		C.bool(verbose))
	input.mutex.RUnlock()
	if writeRes == -1 {
...
func openOutput(outputFormat string, outputURL string, audio bool, verbose bool,
	input *Input) *C.struct_VSOutput {
	outputFormatC := C.CString(outputFormat)
	outputURLC := C.CString(outputURL)

	input.mutex.RLock()
	output := C.vs_open_output(outputFormatC, outputURLC, input.vsInput,
		C.bool(audio), C.bool(verbose))
	input.mutex.RUnlock()
	if output == nil {
